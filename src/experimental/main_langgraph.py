...
from orchestrator.langgraph_orchestrator import run_langgraph_pipeline as advance_node
import os
import tempfile
import pandas as pd
import sqlite3
...
with st.sidebar.expander("🗄️ Advanced Settings: Database", expanded=False):
    uploaded_db = st.file_uploader("Upload SQLite DB file", type=["db", "sqlite"], key="sqlite_upload", help="Upload a SQLite database file to use for reference data.")
...
with st.sidebar.expander("🌐 GitHub Upload (Optional)", expanded=False):
    ...
    repo_name = st.text_input("GitHub Repo Slug", help="Format: username/repo (e.g., myuser/myrepo)")
...
<div style='text-align: center; margin-top: 10px; margin-bottom: 40px;'>
    ...
    <div style='font-size: 48px; font-weight: 900; color: white; letter-spacing: -1px;'>AIFlowCraft (LangGraph)</div>
...
user_input_raw = st.text_area("📥 Enter your project brief:", key="user_input_text", help="Enter a brief description of your project.")
user_input = strip_unicode_emojis(user_input_raw)
user_file = st.file_uploader("Upload a Word/PDF file", type=["pdf", "docx"], key="doc_file")
